...
(function(){
  const BASE = '/video';
  const API = {
    list: BASE + "/api/v1/video/favorite?page={page}&per_page={size}&sort={sort}",
    remove: (id) => `${BASE}/api/v1/video/${encodeURIComponent(id)}/favorite`,
  };
  const PAGE_SIZE = 12;
  const state = {
  ...
  function card(v){
    ...
    a.innerHTML=`<a class="block relative group" href="${escapeAttr(v.url||`${BASE}/${encodeURIComponent(v.uuid||'')}`)}" aria-label="${escapeAttr(v.title||'Video')}">
  ...
  function row(v){
    const a=document.createElement('a'); a.className='card p-3 flex gap-3 items-center hover:shadow-xl transition-shadow'; a.href=v.url||`${BASE}/${encodeURIComponent(v.uuid||'')}`;
  ...
  function thumb(v){
    if (v.uuid) return `${BASE}/api/v1/video/thumbnails/${encodeURIComponent(v.uuid)}.jpg`;
